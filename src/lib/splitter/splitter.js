...
(function (factory, undefined) {
    ...
    $.fn.split = function (options) {
        ...
        var settings = $.extend({
            ...
            onDrag: $.noop,
            percent: false,
            height_unit: "px",  // 컨테이너 높이값을 고정(px), 유동(유무)
            cls_vspltter: "vsplitter",
            cls_hspltter: "hsplitter",
        }, options || {});
        ...
        if (children.length === 2) {
            if (settings.orientation == 'vertical') {
                ...
                cls = settings.cls_vspltter;
            } else if (settings.orientation == 'horizontal') {
                panel_1 = children.first().addClass('top_panel');
                panel_2 = panel_1.next().addClass('bottom_panel');
                cls = settings.cls_hspltter;
            }
        ...
        // 고정/유동 옵션 처리
        if (parent.length) {
            if (settings.height_unit == '%') {   // 유동으로 변경
                var h = parent[0].style.height;
                this.height(h);
            } else {                    // px단위로 변경
